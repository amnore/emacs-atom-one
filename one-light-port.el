...
(defun one-light-port--mix (color1 color2 &optional weight)
  "Mix COLOR1 and COLOR2, COLOR1 has a weight of WEIGHT, COLOR2 has (1 - WEIGHT).
If WEIGHT is not specified, default to 0.5"
  (let ((color1-rgb (apply 'color-hsl-to-rgb color1))
    ...
    (apply 'color-rgb-to-hsl
           (mapcar (lambda (color1 color2) (+ (* color1 w) (* color2 (- 1 w))))
                   (list color1-rgb color2-rgb)))))

(defun one-light-port--luma (color)
  "Calculate the luma of COLOR."
  (let* ((rgb (apply 'color-hsl-to-rgb color))
         (rgb-linear (mapcar (lambda (u) (if (< u 0.3928) (/ u 12.92)
                                           (math-pow (/ (+ u 0.055) 1.055) 2.4)))
                             rgb)))
    (cl-reduce '+ (mapcar (lambda (w u) (* w u))
...
(defun one-light-port--color-hex-to-hsl (color)
  ...
  (let* ((digits (/ (length color) 3))
         (hex-max (- (math-pow 16.0 digits) 1)))
    (unless (and (= (- (length color) 1) (* digits 3))
                 (cl-find digits '(1 2 4))
                 (= (nth 0 color) ?#))
      (error "Invalid hex color"))
    (apply 'color-rgb-to-hsl
           (mapcar (lambda (x) (/ (string-to-number x) hex-max))
                   (mapcar (lambda (pos) (substring (+ 1 (* digits pos))
                                                    (+ 1 (* digits (+ 1 pos)))))
                           '(0 1 2))))))
...
(let* ((to-ratio (lambda (degree) (one-light-port--degree-to-ratio degree)))
       (mix (lambda (c1 c2 w) (one-light-port--mix c1 c2 w)))
       (darken (lambda (color ratio) ((apply-partially 'color-darken-hsl color) (* ratio 100))))
       (lighten (lambda (color ratio) ((apply-partially 'color-lighten-hsl color) (* ratio 100))))
       (luma (lambda (color) (one-light-port--luma color)))
       (name-to-hsl (lambda (name) (apply 'color-rgb-to-hsl (color-name-to-rgb name))))
       (hsv-to-hsl (lambda (hsv) (one-light-port--color-hsv-to-hsl hsv)))
       (hex-to-hsl (lambda (hex) (one-light-port--color-hex-to-hsl hex)))
       (contrast (lambda (color dark light &optional threshold) (one-light-port--color-contrast color dark light threshold)))
       (hue (lambda (hsl) (nth 0 hsl)))
       (saturation (lambda (hsl) (nth 1 hsl)))
       (lightness (lambda (hsl) (nth 2 hsl)))
       ...
       (inset-panel-border-color (fadeout base-border-color 0.15))
       ...
       (tab-height 30)
       ;; Sizes (Custom)
       (ui-size 1) ; TODO: use the correct size
  ...
  (custom-theme-set-faces 'one-light-port
                          (list `(default (,display . (:foreground ,syntax-fg :background ,syntax-bg))))))
